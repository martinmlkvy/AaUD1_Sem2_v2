...
#include "array_list.h"
	...
	{
		//mazanie struktur postOrder
		for (auto son : *children_)
		{
			delete son;
		}
		delete children_;
	...
	inline void MultiWayTreeNode<T>::insertSon(TreeNode<T>* son, int order)
	{
		...
		{
			throw std::logic_error("MultiWayTreeNode<T>::insertSon failed!");
		}
	}
	...
	inline TreeNode<T>* MultiWayTreeNode<T>::replaceSon(TreeNode<T>* son, int order)
	{
		try
		{
			TreeNode<T>* originalSon = (*children_)[order];
		...
		{
			throw std::logic_error(" MultiWayTreeNode<T>::replaceSon failed!");
		}
	}
	...
	{
		TreeNode<T> * out = children_->removeAt(order);
		out->resetParent();
		return out;
	}
