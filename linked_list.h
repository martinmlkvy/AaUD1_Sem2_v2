...
inline LinkedList<T>::LinkedList(const LinkedList<T>& other) :
	LinkedList() //volanie imnplicitneho konst
{
	*this = other; //zavolali sme operator priradenia 
}
...
inline LinkedList<T>& LinkedList<T>::operator=(const LinkedList<T>& other)
{
	if (this != &other)
	{
		//1. zmaz data (clear)
		clear();
		//2. size = other.size toto nie lebo by som mal dva krat tlko prvkov kvoli add
		//size_ = other.size_;
		//3. prejdi vsetky prvky v other a prirad na koniec (add)
		LinkedListItem<T>* current = other.first_; //to je vlasten ten jeden item 
		while (current != nullptr)
		{
			add(current->accessData()); //size zvysime v add, robime copie prvkov 
...
{
	//priddava data na koniec
	/*1. vytvor novy LLitem
	2. last->setNext(novy)
		a)ale ked je zoznam prazdny tak treba first=last=novy
	3. last = novy
	4. ++size_
	*/
...
{
	/*
	na zaciatok(index = 0)/ do stredu (index= nieco)/ na koniec(index= size_)
	*/
	//DSRoutines::rangeCheckExcept(index, size_ + 1, "Invalid index!"); //+1 aby som mohol pridat na koniec, kontroly sa robia pocas behu
	if (index == size_)
	{
		add(data);
	}
	...
	{
		/*
		1. vytvorit novy LLitem
		2. predchadzajuci = prvok na index -1 (getItemAtIndex)
		3.novy-> setnext(pred.getnext())
			pred->setNext(novy) --musi to byt v takomto poradi

		Ked budem na zaciatku
			- novy->setNext(first)
			- first = novy
		*/
...
inline bool LinkedList<T>::tryRemove(const T & data)
{
	...
	//nieco ako v array_liste
}

template<typename T>
inline T LinkedList<T>::removeAt(const int index)
{
	DSRoutines::rangeCheckExcept(index, size_, "invladi index!");
	...
	{
		LinkedListItem<T>* pred = getItemAtIndex(index - 1);
		deleted = pred->getNext();
		pred->setNext(deleted->getNext());
		if (last_ == deleted)
		{
			last_ = nullptr;
		}
...
inline int LinkedList<T>::getIndexOf(const T & data)
{
	...
	//budeme prechadzat pokial nenajdeme data
}

template<typename T>
inline void LinkedList<T>::clear()
{
	while (size_ != 0) //z predka Structure, size_ == 0 je to iste
	{
		removeAt(0);
	}
}
...
inline LinkedListItem<T>* LinkedList<T>::getItemAtIndex(int index) const
{
	...
	{
		result = result->getNext(); //skaceme postupne po prvkoch az pokial neprideme po ten index 
	}
	return result;
}
...
{
	const LinkedListIterator& otherALI = dynamic_cast<const LinkedListIterator&>(other);
...
{
	const LinkedListIterator& otherALI = dynamic_cast<const LinkedListIterator&>(other);
	return position_ != otherALI.position_;
}
