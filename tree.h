...
{
	//prehliadka preorder
	TreeNode<T>* result = shallowCopy(); //akcia
	for (size_t i = 0; i < degree(); i++) //spracovavam mnozinu
	{
		TreeNode<T>* son = getSon(i); //vytiahnem si svojho syna
		if (son != nullptr)
		{
			result->replaceSon(son->deepCopy(), i); //rekurzia
		}
	}
	return result;
}
...
inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
{
	...
	{
		throw std::logic_error("Root does not have any brother!");
	}
}
...
{
	return root_ == nullptr; //keby sme dali size() == 0 tak by sme mali zlozitost O(n) takto mame O(1)
}
...
{
	TreeNode<T>* originalRoot = root_; //aby sme nestratili to co sme mali 
	root_ = newRoot;
	return originalRoot;
}
...
{
	const TreeIterator& otherIT = dynamic_cast<const TreeIterator&>(other);
	*TreeIterator::path_ = *otherIT.path_;
	return *this;
}
...
{
	const TreeIterator& otherIT = dynamic_cast<const TreeIterator&>(other);
	return *TreeIterator::path_ != *otherIT.path_;
}
...
inline void Tree<T>::PreOrderTreeIterator::populatePath(TreeNode<T>* const current)
{
	...
	if (current != nullptr)
	{
		PreOrderTreeIterator::path_->push(current);
...
inline void Tree<T>::PostOrderTreeIterator::populatePath(TreeNode<T>* const current)
{
	if (current != nullptr)
	{
		...
		PreOrderTreeIterator::path_->push_back(current);
	}
}
