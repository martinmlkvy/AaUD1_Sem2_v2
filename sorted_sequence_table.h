...
inline void SortedSequenceTable<K, T>::insert(const K & key, const T & data)
{
	...
	{
		throw std::invalid_argument("Key already present in table");
	}
}
...
{
	bool found = false;
	int index = indexOfKey(key, 0, SequenceTable<K, T>::list_->size(), found); //dal som prec - 1
	return found ? (*SequenceTable<K, T>::list_)[index] : nullptr; //netreba dereferencovat lebo vraciame ukazovatel
}
...
inline int SortedSequenceTable<K, T>::indexOfKey(const K & key, int indexStart, int indexEnd, bool & found) const
{
	...
	{
		if ((*SequenceTable<K, T>::list_)[stred]->getKey() == key)
		{
			found = true;
			return stred;
		}
		if ((*SequenceTable<K, T>::list_)[stred]->getKey() < key)
		{
			return indexOfKey(key, stred + 1, indexEnd, found);
		}
		else
		{
			return indexOfKey(key, indexStart, stred, found);
		}
	}
}
